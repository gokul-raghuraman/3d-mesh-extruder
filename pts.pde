...
class pts { // class for manipulaitng and sisplaying polyloops
 ...
 pt[] G = new pt [maxnv];           // geometry table (vertices)
  ...
  pts movePicked(Vector3d V) { G[pv].add(V); return this;}      // moves selected point (index p) by amount mouse moved recently
  pts moveAll(Vector3d V) {for (int i=0; i<nv; i++) G[i].add(V); return this;};   
